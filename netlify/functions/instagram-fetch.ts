...
function getInsightMetrics(mediaType: string, productType?: string): string {
  const base = "reach,saved,shares,follows,profile_visits,impressions";
  if (mediaType === "VIDEO") {
    if (productType === "REELS") return `${base},ig_reels_video_view_total,ig_reels_avg_watch_time`;
    return `${base},video_views`;
  }
  return base;
}
...
export const handler = async () => {
  ...
  for (const post of posts) {
    try {
      ...
      const metricsRow = {
        ...
        platform: "instagram",
        views: get("video_views") || get("ig_reels_video_view_total"),
        reach: get("reach"),
  ...
  try {
    const acctResp = await fetch(
      `${GRAPH}/${igUserId}?fields=followers_count,following_count,media_count,username&access_token=${TOKEN}`
    );
    ...
    const acctInsightsResp = await fetch(
      `${GRAPH}/${igUserId}/insights?metric=reach,impressions,profile_views,website_clicks&period=day&since=${since}&until=${until}&access_token=${TOKEN}`
    );
    ...
    const summaryRow = {
      ...
      period_impressions: getAcct("impressions"),
      period_profile_views: getAcct("profile_views"),
      period_website_clicks: getAcct("website_clicks"),
